...
void SobelFilter::CopyIn(ID3D12Resource* input)
{
...
void SobelFilter::Execute()
{
	...
	gCommandList->SetPipelineState(gPSOs["Sobel"].Get());
...
void SobelFilter::CopyOut(ID3D12Resource* output)
{
...
void SobelFilter::BuildDescriptor()
{
	// 创建SRV堆
	...
	mOutputCpuUav = hCpuDescriptor.Offset(1, gSetting.CbvSrvUavDescriptorSize);

	mInputGpuSrv = hGpuDescriptor;
	mOutputGpuUav = hGpuDescriptor.Offset(1, gSetting.CbvSrvUavDescriptorSize);
...
void SobelFilter::BuildShader()
{
	gShaders["SobelCS"] = d3dUtil::CompileShader(L"Shaders\\Sobel.hlsl", nullptr, "SobelCS", "cs_5_1");
}

void SobelFilter::BuildPSO()
{
	D3D12_COMPUTE_PIPELINE_STATE_DESC sobelPSO = {};
	sobelPSO.pRootSignature = mRootSignature.Get();
	sobelPSO.CS =
	{
		reinterpret_cast<BYTE*>(gShaders["SobelCS"]->GetBufferPointer()),
		gShaders["SobelCS"]->GetBufferSize()
	};
	sobelPSO.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
	ThrowIfFailed(gD3D12Device->CreateComputePipelineState(&sobelPSO, IID_PPV_ARGS(&gPSOs["Sobel"])));
}